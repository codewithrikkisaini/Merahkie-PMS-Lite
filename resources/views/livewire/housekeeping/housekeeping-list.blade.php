                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex justify-end space-x-2">
                                ...
                                <button wire:click="edit({{ $record->id }})" class="text-xs bg-indigo-500 hover:bg-indigo-600 text-white px-2 py-1 rounded transition-colors" title="Edit"><i class="fas fa-edit"></i></button>
                                @if(auth()->check() && auth()->user()->role && auth()->user()->role->name === 'Admin')
                                <button wire:click="delete({{ $record->id }})" wire:confirm="Are you sure you want to delete this task?" class="text-xs bg-gray-500 hover:bg-gray-600 text-white px-2 py-1 rounded transition-colors" title="Delete"><i class="fas fa-trash-alt"></i></button>
                                @endif
                            </div>
                        </td>
                    </tr>
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="store">
                    <div class="bg-blue-500 px-4 py-3 flex items-center justify-between">
                        <h3 class="text-lg font-medium text-white" id="modal-title">{{ $edit_id ? 'Edit Housekeeping Task' : 'Add Housekeeping Task' }}</h3>
                    ...
                    <div class="bg-white px-6 py-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea wire:model="notes" id="notes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm mt-1"></textarea>
                    ...
                    <div class="bg-white border-t border-gray-100 px-4 py-3 flex justify-end space-x-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-md hover:bg-blue-600 focus:outline-none transition-colors">Save Task</button>
                    </div>
                </form>
            </div>
        </div>
